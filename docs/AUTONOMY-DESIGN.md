# Autonomy Design: Safety Posture

## Context

The central design question in ApprovalFlow is how much authority to give the automated approval path.
...
For the local runtime, ApprovalFlow uses a conservative autonomy posture configured through `data/policy-config.json`:
