...
> This policy defines the expense and invoice rules used by ApprovalFlow.
> It is designed to be readable by human reviewers and consumable by the agent service.
> Each rule has a stable `rule_id` so recommendations and audit records can cite the
> specific policy rule that influenced a decision.
> The agent may retrieve only relevant rules for a submission, while the deterministic
> router remains the final authority for workflow routing and autonomy controls.
